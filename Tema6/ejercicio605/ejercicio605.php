<?php
// 606campeones.php - Lista de campeones de League of Legends con opciones de editar y borrar
...
<head>
    ...
    <script>
        function confirmarBorrado(id, nombre) {
            if (confirm('¿Estás seguro de que quieres borrar al campeón "' + nombre + '"? Esta acción no se puede deshacer.')) {
                window.location.href = 'borrar_campeon.php?id=' + id;
            }
        }
    </script>
</head>
<body>
    ...
    if ($result && $result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) {
            ...
            echo "<button onclick=\"window.location.href='605editando.php?id=" . $row['id'] . "'\">Editar</button>";
            echo "<button onclick=\"confirmarBorrado(" . $row['id'] . ", '" . addslashes($row['nombre']) . "')\">Borrar</button>";
            echo "</div>";
